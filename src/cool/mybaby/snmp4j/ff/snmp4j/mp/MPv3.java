...
import cool.mybaby.snmp4j.ff.snmp4j.asn1.BERSerializable;
import cool.mybaby.snmp4j.ff.snmp4j.event.CounterEvent;
import cool.mybaby.snmp4j.ff.snmp4j.event.SnmpEngineEvent;
import cool.mybaby.snmp4j.ff.snmp4j.event.SnmpEngineListener;
...
/**
 * The <code>MPv3</code> is the message processing cool.mybaby.snmp4j.model for SNMPv3.
 *
 * @author Frank Fock
 * @version 1.0
 */
public class MPv3
     ...
     *
     * @param localEngineID the local engine ID. Its length must be >= 5 and <= 32.
     */
     ...
     *
     * @param engineID the local engine ID. Its length must be >= 5 and <= 32.
     */
     ...
     *
     * @param id an authentication protocol OID.
     * @return an <code>AuthenticationProtocol</code> instance if the supplied ID
     ...
     *
     * @param id an privacy protocol OID.
     * @return an <code>PrivacyProtocol</code> instance if the supplied ID
     * is supported, otherwise <code>null</code> is returned.
     */
    ...
    /**
     * Gets the security cool.mybaby.snmp4j.model for the supplied ID.
     *
     * @param id a security cool.mybaby.snmp4j.model ID.
     * @return a <code>SecurityModel</code> instance if the supplied ID
     * is supported, otherwise <code>null</code> is returned.
     */
     ...
     *
     * @param address  the <code>Address</code> of the remote SNMP engine.
     * @param engineID the engine ID of the remote SNMP engine.
     */
     ...
     *
     * @param address the <code>Address</code> of the remote SNMP engine.
     * @return the engine ID of the remote SNMP engine or <code>null</code> if there
     ...
     *
     * @param address the <code>Address</code> of the remote SNMP engine for whose engine ID
     *                is to be removed.
     * @return the removed engine ID of the remote SNMP engine or <code>null</code> if
     ...
     * for the MPv3 message processing cool.mybaby.snmp4j.model for a single message.
     *
     * @author Frank Fock
     * @version 1.0
     */
    ...
    protected static class Cache {
         ...
         *
         * @param entry the state reference to add.
         * @return {@link SnmpConstants#SNMP_MP_DOUBLED_MESSAGE} if the entry already
...
*/
         ...
         *
         * @param pduHandle a pduHandle.
         * @return <code>true</code> if an entry has been deleted, <code>false</code>
         * otherwise.
         */
         ...
         * @return a <code>CacheEntry</code> instance with the given message ID or
     ...
     *
     * @return a message ID >= 1 and <= {@link #MAX_MESSAGE_ID}.
     */
     ...
     *
     * @return return a <code>SecurityProtocols</code>.
     */
    ...
    /**
     * Gets the default security cool.mybaby.snmp4j.model to be used for engine ID discovery.
     *
     * @return a security cool.mybaby.snmp4j.model ID.
     * @see USM#getID()
     */
     ...
     *
     * @param messageDispatcher        Send the message on behalf the supplied MessageDispatcher instance.
     * @param pdu                      ScopedPDU
     ...
     * @param securityLevel            The security level to use when sending this report.
     * @param securityModel            The security cool.mybaby.snmp4j.model to use when sending this report.
     * @param securityName             The security name to use when sending this report.
     * @param maxSizeResponseScopedPDU the maximum size of of the report message (will be most likely ignored
     *                                 because a report should always fit in 484  bytes).
     * @param payload                  the variable binding to include in the report message.
     * @param stateReference           the state reference associated with the original message.
    ...
    public int prepareDataElements(MessageDispatcher messageDispatcher,
        ...
        try {
            ...
            if (secModel == null) {
                logger.error("RFC3412 �7.2.4 - Unsupported security cool.mybaby.snmp4j.model: " +
                        securityModel);
                CounterEvent event =
            ...
            if (status != SnmpConstants.SNMPv3_USM_OK) {
                ...
                return SnmpConstants.SNMP_MP_USM_ERROR;
            }
                ...
                if (cacheEntry != null) {
                    ...
                    if (scopedPdu.getType() == PDU.REPORT) {
                        ...
                        if (((cacheEntry.getSecurityEngineID().length != 0) &&
                                (!securityEngineID.equals(cacheEntry.getSecurityEngineID()))) ||
                                (secModel.getID() != cacheEntry.getSecurityModel().getID()) ||
                                ((!securityName.equals(cacheEntry.getSecurityName()) &&
                                        (securityName.length() != 0)))) {
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "RFC 3412 �7.2.11 - Received report message does not match sent message");
                            }
                        ...
                        addEngineID(cacheEntry.getAddress(), securityEngineID);
                        //cache.deleteEntry(cacheEntry.getPduHandle());
                        mutableStateReference.setStateReference(null);
                    ...
                    if (scopedPdu.getType() == PDU.RESPONSE) {
                        if (((!securityEngineID.equals(cacheEntry.getSecurityEngineID())) &&
                                (cacheEntry.getSecurityEngineID().length != 0)) ||
                                (secModel.getID() != cacheEntry.getSecurityModel().getID()) ||
                                (!securityName.equals(cacheEntry.getSecurityName())) ||
                                (securityLevel.getValue() != cacheEntry.getSecurityLevel()) ||
                                ((!scopedPdu.getContextEngineID().equals(cacheEntry.getContextEngineID())) &&
                                        (cacheEntry.getContextEngineID().length != 0)) ||
                                ((!scopedPdu.getContextName().equals(cacheEntry.getContextName()) &&
            ...
            switch (scopedPdu.getType()) {
                ...
                case PDU.SET: {
                    ...
                    } else if (!securityEngineID.equals(localEngineID)) {
     ...
     *
     * @param securityModels a <code>SecurityModels</code> instance.
     */
     ...
     *
     * @return a <code>SecurityModels</code> instance.
     */
     ...
     *
     * @return an enterprise ID as registered by the IANA (see http://www.iana.org).
     */
     ...
     *
     * @param newEnterpriseID an enterprise ID as registered by the IANA (see http://www.iana.org).
     */
     ...
     *
     * @return a <code>CounterSupport</code> instance that is used to fire
     * {@link CounterEvent}.
     */
     ...
     *
     * @param counterSupport a <code>CounterSupport</code> subclass instance.
     */
     ...
     * the engine ID cache.
     *
     * @param l a <code>SnmpEngineListener</code> instance.
     * @since 1.6
     */
    public synchronized void addSnmpEngineListener(SnmpEngineListener l) {
        if (snmpEngineListeners == null) {
            snmpEngineListeners = new Vector();
        }
        snmpEngineListeners.add(l);
    }
     ...
     *
     * @param engineEvent the <code>SnmpEngineEvent</code> instance to fire.
     * @since 1.6
     */
    protected void fireEngineChanged(SnmpEngineEvent engineEvent) {
        if (snmpEngineListeners != null) {
            Vector listeners = snmpEngineListeners;
