...
import cool.mybaby.snmp4j.ff.snmp4j.event.AuthenticationFailureEvent;
import cool.mybaby.snmp4j.ff.snmp4j.event.AuthenticationFailureListener;
import cool.mybaby.snmp4j.ff.snmp4j.event.CounterEvent;
import cool.mybaby.snmp4j.ff.snmp4j.event.CounterListener;
...
import cool.mybaby.snmp4j.ff.snmp4j.smi.GenericAddress;
import cool.mybaby.snmp4j.ff.snmp4j.smi.Integer32;
import cool.mybaby.snmp4j.ff.snmp4j.smi.OctetString;
import cool.mybaby.snmp4j.ff.snmp4j.transport.UnsupportedAddressClassException;
...
public class MessageDispatcherImpl implements MessageDispatcher {
    ...
    /**
     * Adds a message processing cool.mybaby.snmp4j.model to this message dispatcher. If a message
     * processing cool.mybaby.snmp4j.model with the same ID as the supplied one already exists it
     * will not be changed. Please call {@link #removeMessageProcessingModel}
     * before to replace a message processing cool.mybaby.snmp4j.model.
     *
     * @param model a MessageProcessingModel instance.
     */
    ...
    /**
     * Removes a message processing cool.mybaby.snmp4j.model from this message dispatcher.
     *
     * @param model a previously added MessageProcessingModel instance.
     */
     ...
     *
     * @param transport a TransportMapping instance. If there is already another transport
     ...
     *
     * @param transport a previously added TransportMapping instance.
     ...
     * @param destAddress the transport address where to send the message. The
     *                    <code>destAddress</code> must be compatible with the supplied
     *                    <code>transport</code>.
     * @param message     the SNMP message to send.
     * @throws IOException if an I/O error occured while sending the message or if there is
     ...
     *
     * @param sourceTransport a <code>TransportMapping</code> that matches the incomingAddress type.
     * @param mp              a <code>MessageProcessingModel</code> to process the message.
     * @param incomingAddress the <code>Address</code> from the entity that sent this message.
     * @param wholeMessage    the <code>BERInputStream</code> containing the SNMP message.
    ...
    public PduHandle sendPdu(TransportMapping transport,
        ...
        try {
            ...
            if (mp == null) {
                throw new MessageException("Unsupported message processing cool.mybaby.snmp4j.model: "
            ...
            if (!mp.isProtocolVersionSupported(messageProcessingModel)) {
                throw new MessageException("SNMP version " + messageProcessingModel +
                        " is not supported " +
                        "by message processing cool.mybaby.snmp4j.model " +
                        messageProcessingModel);
            }
            if (transport == null) {
            ...
            if (status == SnmpConstants.SNMP_ERROR_SUCCESS) {
                ...
                throw new MessageException("Message processing cool.mybaby.snmp4j.model " +
     ...
     *
     * @param transportAddress       the target address.
     * @param messageProcessingModel the message processing cool.mybaby.snmp4j.model to be used.
     * @param pdu                    the PDU to be sent.
     * @throws MessageException if unrecoverable inconsistencies have been detected.
     */
    ...
    public void releaseStateReference(int messageProcessingModel,
        ...
        if (mp == null) {
            throw new IllegalArgumentException("Unsupported message processing cool.mybaby.snmp4j.model: " +
                    messageProcessingModel);
        }
     ...
     * PDU successfully.
     *
     * @param e a <code>CommandResponderEvent</code> event.
     */
     ...
     * processing cool.mybaby.snmp4j.model ID.
     *
     * @param messageProcessingModel a message processing cool.mybaby.snmp4j.model ID
     ...
     *
     * @param counterListener a previously added <code>CounterListener</code>.
     */
     ...
     *
     * @param counterListener a <code>CounterListener</code> that will be informed when a counter
     ...
     *
     * @param event the <code>CounterEvent</code> containing the OID of the counter
     ...
     *
     * @param checkOutgoingMsg if <code>true</code> outgoing messages are checked for consistency.
     ...
     *
     * @return if <code>true</code> outgoing messages are checked for consistency.
     ...
     * incoming messages.
     *
     * @param l the <code>AuthenticationFailureListener</code> to add.
     * @since 1.5
     */
     ...
     *
     * @param l the <code>AuthenticationFailureListener</code> to remove.
     */
